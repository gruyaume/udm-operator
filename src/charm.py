...
class UDMOperatorCharm(CharmBase):
    """Main class to describe juju event handling for the 5G UDM operator."""
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(self.on.udm_pebble_ready, self._on_udm_pebble_ready)
        self.framework.observe(self.on.nrf_relation_created, self._on_udm_pebble_ready)
        self.framework.observe(self._nrf_requires.on.nrf_available, self._on_nrf_available)

        self._service_patcher = KubernetesServicePatch(
            charm=self,
            ports=[
                ServicePort(name="sbi", port=29503),
            ],
        )
    ...
    def _on_nrf_available(self, event: NRFAvailableEvent) -> None:
        if not self._container.can_connect():
            ...
            event.defer()
            return
        self._write_config_file(nrf_url=event.url)
        self._on_udm_pebble_ready(event)

    def _write_config_file(self, nrf_url: str) -> None:
        jinja2_environment = Environment(loader=FileSystemLoader("src/templates/"))
        template = jinja2_environment.get_template("udmcfg.conf.j2")
    ...
    def _on_udm_pebble_ready(self, event: Union[PebbleReadyEvent, NRFAvailableEvent]) -> None:
